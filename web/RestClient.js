...
// Model for TestUnitsEntity entity
    models.TestUnitsEntity = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/sr/web/testunits/",
        idAttribute: 'id',
        defaults: {
            topoid: "",
            description: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/sr/web/testunits/';
            }
    ...
    // Collection class for TestUnitsEntity entities
    models.TestUnitsEntityCollection = Backbone.Collection.extend({
        model: models.TestUnitsEntity,
        url: "http://localhost:8080/sr/web/testunits/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.TestUnitsEntityCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-testunitsentity-list-itemis template identifier for item
                        templateName: '#tpl-testunitsentity-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.testunitsentity = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.testunitsentity,
                    // tpl-testunitsentity-details is template identifier for chosen model element
                    templateName: '#tpl-testunitsentity-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.TestUnitsEntity();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-testunitsentity-details is a template identifier for chosen model element
                templateName: '#tpl-testunitsentity-details',
        ...
        getData: function () {
            return {
                ...
                testUnitName: $('#testUnitName').val(),
                jobreqAgentCommand: $('#jobreqAgentCommand').val()
            };
        }
    });
    new AppRouter();
